		...
		$(function(){
		 ...
		 $("#last-name").change(function(){
		    ...
		    var p=$(this).children('option:selected').val();//这就是selected的值 
		 	...
		 	$.ajax({
		 	async: false,
	   	 url: "toGrantUser.do?id="+p,
	   	 dataType:"json",
	     success: function(msg){
   		var node="";
   		for(var i=0;i<msg.length;i++){
   			node =node+ "<li class='dd-item' data-id='"+msg[i].id+"'><div class='dd-handle'>"+msg[i].name+"</div>";
   			if(msg[i].childs!=null||msg[i].childs!=""){
   				var child=msg[i].childs;
   				node=node+"<ol class='dd-list dd-nodrag'>";
   				for(var j=0;j<child.length;j++){
   					node=node+"<li class='dd-item' data-id='"+child[j].id+"'><div class='dd-handle'>"+child[j].name+"</div></li>";
   				}
   				node=node+"</ol>"
   			}
   			node=node+"</li>";
			 			...
			 			<div class="x_content">
			 			   <form  class="form-horizontal form-label-left"  >
			 					<div class="form-group">
			 						<label class="control-label col-md-3 col-sm-3 col-xs-12" for="last-name">要设置的角色dfsdf:
                                            </label>
			 		...
			 		<div class="x_panel">
						...
						<div class="x_content">
							<div class="dd"  id="nestable_list_2" >
								<ol class="dd-list dd-nodrag" id="ol2">
								</ol>
							</div>
						</div>
			 		</div>
